...
@Autonomous(name = "Simplified Odometry Inclusive", group="Robot")
public class SimplifiedOdometryInclusive extends LinearOpMode{
    // Adjust these numbers to suit your robot.
    private final double ODOM_INCHES_PER_COUNT   = 0.002969;   //  GoBilda Odometry Pod, 48mm wheel dia., 2000 ticks/revolution, (1/326.8)
     ...
     */
    public void driveRobot(String direction, double distanceInches, double power, double holdTime) {
        ...
        //driveControlNewDistance(distanceInches, power = power * (dirFlag ? -1 : 1));
        driveControlNewDistance(directionInches, power);   // achieve desired drive distance
     ...
     * @param holdTime Minimum time (sec) required to hold the final position.  0 = no hold.
     */
    public void strafeRobot(String direction, double distanceInches, double power, double holdTime) {
        ...
        strafeControlNewDistance(directionInches, power);  // Achieve desired Strafe distance
        yawControlReset();                          // Maintain last turn angle
        holdTimer.reset();
    ...
    public void turnToHeading(double headingDeg, double power, double holdTime) {
        ...
        yawControlNewHeading(headingDeg, power);
